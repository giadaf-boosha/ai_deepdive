...
C'e' un'insidia strutturale: i cammini ridondanti. Lo stesso stato puo' essere raggiunto per vie diverse, e i cicli possono rendere infinito l'albero anche quando lo spazio degli stati e' minuscolo. La sintesi del libro sta in un detto memorabile: gli algoritmi che non ricordano il passato sono condannati a ripeterlo. Le contromisure sono tre: memorizzare tutti gli stati gia' raggiunti (ricerca su grafo), ignorare il problema quando la struttura del dominio lo consente (ricerca ad albero), o controllare solo i cicli risalendo la catena dei nodi padre. E' un trade-off puro tra memoria e tempo.
...
- A* con euristica ammissibile e' completa, ottima rispetto al costo e ottimamente efficiente; la consistenza rafforza le garanzie e semplifica l'implementazione.
