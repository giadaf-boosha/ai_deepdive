---
title: "20 — Tips & best practices"
slug: 20-tips-best-practices
order: "020"
source: https://github.com/giadaf-boosha/claude-code/blob/main/docs/20-tips-best-practices.md
---
...
**Per il deep-dive**: [22 — Compound engineering](/claude-code/22-compound-engineering) per come comporre i tips in pattern superiori.
...
Vedi [07 Hooks § esempi](/claude-code/07-hooks#7.8-esempi-pratici).
...
> Nota 2026: dalla v2.1.38+ esiste `claude --worktree` nativo (vedi [docs/19 § Fase 4](/claude-code/19-changelog)). Lo script bash resta utile per setup massivo o pre-2.1.
...
- Checkpoint umano dopo il piano, prima dell'esecuzione (vedi [22 § R4](/claude-code/22-compound-engineering))
...
Vedi anche [docs/16 § Headless mode](/claude-code/16-headless-agent-sdk) per esempi piu' avanzati con `--output-format stream-json`.
...
Tre sub-agent (vedi [docs/08](/claude-code/08-subagents)) lavorano in serie con contesti isolati. Il main thread vede solo i riassunti, non i contesti pieni — risparmio token significativo su task lunghi.
...
- **Hook PostToolUse "Lint after Edit"**: linter automatico solo sui file modificati.
  ...
  ```
- **Hook PreToolUse "Bash validator"**: blocca comandi pericolosi (es. `rm -rf`, `curl | sh`) prima dell'esecuzione passando un validator Python che ritorna exit code 2. Pattern simmetrico al "file protection hook" di [docs/07](/claude-code/07-hooks).
...
← [19 Changelog](/claude-code/19-changelog) · Torna al [README](/claude-code)
