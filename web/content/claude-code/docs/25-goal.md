---
title: "25 — /goal (lavoro autonomo verso una condizione)"
slug: 25-goal
order: "025"
source: https://github.com/giadaf-boosha/claude-code/blob/main/docs/25-goal.md
---
...
> `/goal` e' un wrapper attorno a uno [Stop hook prompt-based](/claude-code/07-hooks) scoped alla sessione. Dopo ogni turno, la condizione e la conversazione finora vengono mandate a un **small fast model** (default Haiku), che ritorna un si'/no + una breve motivazione. Un "no" dice a Claude di continuare (e passa la motivazione come guida per il prossimo turno); un "si'" pulisce il goal. E' il modo di trasformare "lavora fino a che X e' vero" in un loop verificato da un modello terzo, fresco, non quello che fa il lavoro.
...
**Per il deep-dive**: [14 — `/loop` & Monitor](/claude-code/14-loop-monitor) (loop a intervallo di tempo) e [07 — Hooks](/claude-code/07-hooks) (Stop hook custom).
...
| [`/loop`](/claude-code/14-loop-monitor) | Passa un intervallo di tempo | Lo fermi tu, o Claude decide che il lavoro e' fatto |
| [Stop hook](/claude-code/07-hooks) | Il turno precedente finisce | Lo decide il tuo script o prompt |
...
[Auto mode](/claude-code/04-modalita-permessi) da solo approva le tool call **dentro** un turno ma non ne avvia uno nuovo. Sono **complementari**: auto mode toglie i prompt per-tool, `/goal` toglie i prompt per-turno.
...
### Non-interactive (`-p`)
`/goal` funziona in [non-interactive mode](/claude-code/16-headless-agent-sdk), nella [desktop app](https://code.claude.com/docs/en/desktop) e via [Remote Control](https://code.claude.com/docs/en/remote-control). Con `-p` il loop gira fino a completamento in una singola invocazione:
```bash
...
5. **`/goal` per intra-sessione, routine per cross-sessione**: per lavoro che gira senza sessione aperta usa [Routines cloud](/claude-code/13-routines-cloud) o desktop scheduled, non `/goal`.

---

← [24 Dynamic Workflows](/claude-code/24-workflows) · Successivo → [01 Snapshot](/claude-code/01-snapshot)
