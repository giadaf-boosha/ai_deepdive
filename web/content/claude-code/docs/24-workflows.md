---
title: "24 — Dynamic Workflows (orchestrazione subagent su scala)"
slug: 24-workflows
order: "024"
source: https://github.com/giadaf-boosha/claude-code/blob/main/docs/24-workflows.md
---
Un dynamic workflow e' uno **script JavaScript** che Claude scrive per il task che descrivi e che un runtime esegue **in background**, orchestrando da decine a centinaia di [subagent](/claude-code/08-subagents) in parallelo mentre la sessione resta libera.
...
**Per il deep-dive**: [12 — Agent Teams](/claude-code/12-agent-teams) (orchestrazione multi-thread interattiva) e [08 — Subagents](/claude-code/08-subagents) (il primitivo "worker" che i workflow orchestrano).
...
Se vieni interpellato dipende dal [permission mode](/claude-code/04-modalita-permessi):
...
Disponibili in: CLI, Desktop app, IDE extensions, [non-interactive mode](/claude-code/16-headless-agent-sdk) (`claude -p`), Agent SDK. Su tutti i piani a pagamento, con accesso API Anthropic, e su Amazon Bedrock, Google Cloud Vertex AI, Microsoft Foundry. **Su Pro** vanno attivati dalla riga *Dynamic workflows* in `/config`.
...
← [23 Glossario](/claude-code/23-glossario) · Successivo → [25 /goal](/claude-code/25-goal)
