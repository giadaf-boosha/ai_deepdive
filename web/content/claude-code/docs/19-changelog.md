---
title: "19 — Changelog completo Claude Code (feb 2025 → apr 2026)"
slug: 19-changelog
order: "019"
source: https://github.com/giadaf-boosha/claude-code/blob/main/docs/19-changelog.md
---
...
**Per il deep-dive**: [00 — Harness overview](/claude-code/00-harness-overview) per leggere il changelog in chiave IMPACT.
...
> Nota: Sonnet 4.5 e Opus 4.1 sono superati da Sonnet 4.6 / Opus 4.6 / Opus 4.7 (apr 2026). I benchmark sopra sono storici al 29 set 2025. Per i modelli correnti vedi [05 — Fast mode + 1M context](/claude-code/05-fast-mode-1m-context).
...
- **`autoMode.hard_deny`** (v2.1.134–136, 8 mag 2026): nuove regole di blocco assoluto per auto mode — azioni matching vengono bloccate incondizionatamente indipendentemente da eccezioni "allow" configurate. Vedi [04 Permessi](/claude-code/04-modalita-permessi).
...
| 28 mag 2026 | **v2.1.154** | **Opus 4.8** (modello premium) come default per effort `xhigh`, con `xhigh` che diventa il nuovo `high`; **Dynamic Workflows** in research preview via **`/workflows`** per orchestrare decine/centinaia di agenti background (vedi [./24-workflows.md](/claude-code/24-workflows)); **`! <command>`** in Agent View per eseguire comandi in sessione background; Fast Mode su Opus 4.8 (2x rate per 2.5x velocita'); lean system prompt default per Opus 4.8; `/effort` slider rinominato "Faster"/"Smarter"; `CLAUDE_CODE_SESSION_ID` in stdio MCP subprocess |
...
← [18 Settings & Auth](/claude-code/18-settings-auth) · Successivo → [20 Tips & best practices](/claude-code/20-tips-best-practices)
