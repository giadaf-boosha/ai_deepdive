---
title: "Quick Start — primi 60 minuti con Claude Code"
slug: QUICKSTART
order: "099"
source: https://github.com/giadaf-boosha/claude-code/blob/main/docs/QUICKSTART.md
---
...
> Se manca: chiudi il terminale, riapri (per ricaricare PATH). Se ancora manca, vedi [02 § install](/claude-code/02-cli-installazione).
...
> Vedi [18 § authentication precedence](/claude-code/18-settings-auth#185-authentication) per OAuth, Bedrock, Vertex.
...
> Per CLAUDE.md piu' ricco: `CLAUDE_CODE_NEW_INIT=1 claude /init` (flow multi-fase). Vedi [06 — CLAUDE.md & memory](/claude-code/06-claude-md-memory).
...
> Nota: se vedi "Edit", "Bash" o "Write" tool che chiedono permessi, e' la modalita' default `ask`. Approvi una volta o aggiungi alle allow rules. Vedi [04 — Permessi](/claude-code/04-modalita-permessi).
...
> Skill = memoria eseguibile. Ne hai create centinaia? Vedi [09 — Skills](/claude-code/09-skills). Marketplace community: [11](/claude-code/11-plugins-marketplace).
...
> Approfondimento: [07 — Hooks](/claude-code/07-hooks). Pattern Authority: [04b § Layer 3](/claude-code/04b-authority-model#layer-3-—-hooks-custom-logic).
...
> Vedi [14 — `/loop` & Monitor](/claude-code/14-loop-monitor), [14b — Agent loop ReAct](/claude-code/14b-agent-loop-react).
...
|---|---|
| Beginner / non-coder | [README-NAVIGATION § Percorso A](/claude-code/README-NAVIGATION) (15 min altri) |
| Solo dev / Indie hacker | [21 § Indie hacker](/claude-code/21-guide-target-user#2-solo-developer--indie-hacker) |
| Senior backend (con team) | [21 § Senior backend](/claude-code/21-guide-target-user#3-senior-backend-developer-con-team) |
| Frontend / Full-stack | [21 § Frontend](/claude-code/21-guide-target-user#4-frontend--full-stack-developer) |
| DevOps / SRE | [21 § DevOps](/claude-code/21-guide-target-user#5-devops--sre) |
| Tech lead / Architect | [21 § Tech lead](/claude-code/21-guide-target-user#6-tech-lead--architect) |
| AI/ML engineer | [21 § AI/ML](/claude-code/21-guide-target-user#7-aiml-engineer) |
| Legacy stack maintainer | [21 § Legacy](/claude-code/21-guide-target-user#8-legacy-stack-maintainer) |

E per il framework concettuale:
- [00 — Harness overview](/claude-code/00-harness-overview) — capire COSA stai usando
- [00b — Context engineering](/claude-code/00b-context-engineering) — perche' CLAUDE.md vince sui prompt
...
| Permission prompt fastidiosi | `/fewer-permission-prompts` (skill) o passa a sandbox + auto mode (vedi [04](/claude-code/04-modalita-permessi)) |
...
Vuoi un setup gia' configurato per la tua persona? Vedi [examples/personas/](https://github.com/giadaf-boosha/claude-code/blob/main/examples/personas/) — 8 cartelle template `.claude/` con CLAUDE.md, settings, skill, hook esempio.
...
← Torna al [README](/claude-code) · Vedi anche [README-NAVIGATION](/claude-code/README-NAVIGATION)
