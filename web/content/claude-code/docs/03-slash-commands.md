---
title: "03 — Slash commands"
slug: 03-slash-commands
order: "003"
source: https://github.com/giadaf-boosha/claude-code/blob/main/docs/03-slash-commands.md
---
...
**Per il deep-dive**: [09 — Skills](/claude-code/09-skills) per come scrivere i tuoi.
...
| `/agents` | built-in | Gestione [subagents](/claude-code/08-subagents) |
...
| `/deep-research <domanda>` | **Skill** | Workflow bundled di ricerca: fan-out di web search da piu' angolazioni, fetch e cross-check delle fonti, voto su ogni claim, report citato con i claim non verificati gia' filtrati (richiede il WebSearch tool). Vedi [24](/claude-code/24-workflows) |
...
| `/fast [on\|off]` | built-in | Toggle [fast mode](/claude-code/05-fast-mode-1m-context) (Opus 4.6) |
...
| `/goal [condition]` | built-in | Imposta/verifica/azzera una condizione di completamento; Claude lavora su piu' turni fino al raggiungimento, con overlay live tempo/turni/token. Funziona in modalita' interattiva, `-p` e Remote Control (da v2.1.139). Vedi [25](/claude-code/25-goal) |
...
| `/loop [interval] [prompt]` (alias `/proactive`) | **Skill** | Re-run prompt: con interval (cron), senza interval (Claude self-paces), senza prompt (maintenance prompt o `loop.md`). Vedi [14](/claude-code/14-loop-monitor) |
...
| `/plugin` | built-in | Gestione [plugins](/claude-code/11-plugins-marketplace) |
...
| `/remote-control` (alias `/rc`) | built-in | Espone sessione a [Remote Control](/claude-code/17-ide-surface#remote-control) |
...
| `/sandbox` | built-in | Toggle [sandbox mode](/claude-code/04-modalita-permessi#sandbox) |
| `/schedule [description]` (alias `/routines`) | built-in | Gestione [routines](/claude-code/13-routines-cloud) |
...
| `/ultraplan <prompt>` | built-in | [Plan in cloud](/claude-code/15-ultraplan-ultrareview), review browser, execute remote o local |
| `/ultrareview [PR]` | built-in | [Multi-agent cloud code review](/claude-code/15-ultraplan-ultrareview) (3 free runs Pro/Max → 5 mag 2026) |
...
| `/workflows [description]` | built-in | Lista e gestisce i dynamic workflow run; crea ed esegue un workflow dinamico che orchestra decine/centinaia di agenti background; descrivi il task, Claude genera e lancia il workflow (da v2.1.154). Vedi [24](/claude-code/24-workflows) |
...
← [02 CLI](/claude-code/02-cli-installazione) · Successivo → [04 Modalita' permessi](/claude-code/04-modalita-permessi)
