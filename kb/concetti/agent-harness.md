...
Y Combinator pubblica il 31 luglio su GitHub, licenza MIT, QM: l'agent harness che usa internamente per far girare agenti su contabilita', legale, eventi e ingegneria, inclusa la costruzione di QM stesso. E' cloud-first, con Slack e web UI nativi; ogni persona e ogni "room" ottiene memoria, file, gestione credenziali, permessi, cron job, web app e una sandbox durevole propri, e ogni dipendente puo' alternare tra Pi, OpenCode, Codex e Claude Code all'interno dello stesso workspace senza vendor lock-in sul modello. Le feature includono trigger (cron, webhook), file condivisi, connettori per un "company brain" centralizzato, un browser per agenti e artefatti web-app condivisibili tra piu' utenti dello stesso progetto. Rispetto a tutti gli harness gia' mappati in questa scheda — verticali per dominio (Claude Code per coding, ARIS per ricerca) o multi-agente ma single-user (LangGraph, CrewAI) — QM introduce un asse nuovo nella tabella "Varianti": harness multiplayer, multi-modello e multi-dominio, pensato esplicitamente per l'intera organizzazione invece che per un singolo utente o un singolo caso d'uso, e reso open-source da chi lo usa gia' in produzione per il proprio funzionamento interno. Il pattern "ogni persona e ogni room ha la propria sandbox durevole" e' un'implementazione concreta della separazione stato-per-contesto gia' discussa in questa scheda a proposito della memoria e del context management, applicata pero' a scala di organizzazione invece che di singola sessione agente. [Digest 2026-08-02](../../digest/2026/08/02.md)
