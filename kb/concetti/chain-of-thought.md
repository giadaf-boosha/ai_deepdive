...
Due sviluppi della settimana aggiungono capitoli distinti al tema faithfulness/monitorabilita' della CoT e ai salti di capacita' dei reasoning model su task formali. Primo, OpenAI rilascia il 3 settembre GPT-6 Astra con "recurrent depth" (opaque recurrence): invece di scrivere una catena di pensiero lineare in linguaggio naturale, il modello elabora la stessa query piu' volte in un loop interno, lasciando meno tracce leggibili del proprio processo di ragionamento. Buck Shlegeris e Ryan Greenblatt di Redwood Research avvertono che spingere ulteriormente la tecnica rischia di spostare il ragionamento fuori dai canali visibili su cui si basa gran parte del monitoraggio di safety odierno; il chief scientist OpenAI Jakub Pachocki dichiara che l'azienda tratterrebbe lo scaling piuttosto che accettare un'ulteriore perdita di confidenza nel monitoraggio, negando l'intenzione di uno spostamento verso un "neuralese" completamente opaco. Report citati da The Information indicano che Anthropic e Google DeepMind starebbero esplorando tecniche simili. E' la prima istanza concreta, su un modello gia' in produzione, del rischio teorico gia' tracciato nell'aggiornamento del 23 luglio (Jacobian lens, J-space: meno del 10% dell'attivita' interna del modello passa per il canale verbalizzabile): li' il problema era osservare che la CoT scritta non rappresenta l'intero calcolo interno per costruzione; qui un'architettura di prodotto riduce attivamente la quota di ragionamento che passa per quel canale, con un trade-off esplicitamente dichiarato dal lab tra capacita' e monitorabilita' invece che una proprieta' emergente solo osservata post-hoc. Secondo, Anthropic annuncia il 4 settembre che un modello di ricerca interno ha prodotto in 11 giorni, lavorando in gran parte in autonomia, la prima formalizzazione end-to-end machine-checked del Teorema di Fermat in Lean (13 milioni di righe, 29.500 teoremi intermedi, 6 miliardi di token generati da diverse decine di agenti), un task che i matematici stimavano avrebbe richiesto anni di lavoro umano. La svolta tecnica e' l'accesso a Prove2Me, strumento open-source che aiuta gli agenti a scegliere il passo successivo ottimale in un workflow di dimostrazione lungo e ramificato — un pattern di "tool per la navigazione dello spazio di ricerca" distinto dal semplice reasoning testuale, e un caso d'uso diverso dai salti di capacita' su problemi aperti gia' citati nell'introduzione di questa scheda: qui il target e' la formalizzazione verificabile di una dimostrazione gia' nota, non la scoperta di un risultato nuovo. [Digest 2026-09-08](../../digest/2026/09/08.md)
